...
| 22 | **No classification accuracy numbers** | Harness built (`tools/evaluate.py` reports a confusion matrix with deferral separated from error). Still needs a labelled corpus. Depends on item 8. |
| 23 | **No OCR accuracy on REAL documents** | Harness built and measuring: on a rendered fixture CER 4.2% / WER 29.8%, the gap being word-boundary loss. That is a floor on error, not an estimate — real scans have noise, skew, stamps and Devanagari. Depends on item 8. |
...
| 28 | **No extraction accuracy on REAL documents** | Harness built, reporting per-field precision/recall and a dangerous-error rate. On synthetic fixtures it reads 100%, which measures plumbing only. Depends on item 8. |
...
| 39 | **No labelled corpus for evaluation** | The harness is the easy half; transcribing ground truth for real Mumbai deeds is the expensive half. Nothing substitutes for it. Same blocker as item 8. |
| 40 | **Regression gate thresholds are uncalibrated** | `DEFAULT_GATES` in `eval/report.py` uses 5% dangerous errors / 70% recall. They exist so a regression is visible, not because they are the right numbers. Tighten against a real corpus. |
| ~~42~~ | ~~Per-field accept/correct not built~~ | **Closed 2026-08-25.** Accept/correct per field; a correction becomes a claim with `HumanProvenance` and flows into cross-document checks. `GET /api/feedback` reports correction rate by claimed confidence. |
| 43 | **UI is single-case and in-memory** | `ReviewSession` holds one case in process. Multi-case, persistence and real sessions arrive with authentication (item 13). |
| 44 | **Operator task list not in the UI** | The verification orchestrator produces T4/T5 tasks, but the board does not show them yet. |
| 41 | **Reviewer-outcome metrics are session-only** | Correction rate by claimed confidence is now captured (`FeedbackLog`), but in memory and cleared on reset. Persisting it, and feeding it into calibrated confidence, needs real usage over real cases. |
