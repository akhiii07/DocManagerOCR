...
| 42 | **Per-field accept/correct not built** | The UI shows value, confidence and evidence per field; the accept-or-correct interaction is deferred. It is also the feedback signal for confidence calibration, so it matters beyond the UI. Relates to item 41. |
...
| 41 | **No reviewer-outcome metrics** | Agreement, correction rate, review time and confidence calibration all need the Risk Manager UI and real usage. |
