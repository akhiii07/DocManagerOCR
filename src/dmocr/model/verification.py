...
from .claims import ClaimValue
from .common import ConfidenceTier, Severity
...
class VerificationResult(BaseModel):
    ...
    def counts_as_a_check(self) -> bool:
        """Whether the authority actually answered.
        ...
        `NOT_APPLICABLE` counts **when the source returned a value**. That combination
        means the register holds a record and we simply had nothing internal to compare it
        against - which for a presence check (is there a prior charge?) is the entire
        signal. Excluding it made a real CERSAI charge read as NOT_DETERMINABLE, i.e.
        invisible.
        """
        return self.status.is_answered or (
            self.status is VerificationStatus.NOT_APPLICABLE
